...
def plot_scatter_exe(project_dict,taxalist_plot,networktype,filter_ss):
    ...
    for i in pm_res.index:
        taxa1=pm_res.loc[i, 'taxa1']
        taxa2=pm_res.loc[i, 'taxa2']
        if (taxa1 in taxalist_plot) and (taxa2 in taxalist_plot):
            keep.append(i)
    keep=np.asarray(keep)
    pm_res=pm_res.iloc[keep,]
    ...
    image_pdf = fileplace + '/taxa_relationship_difference_between_groups_withintaxa.pdf'
...
def plot_t_squared_exe(project_dict, taxalist_plot, networktype, filter_ss):
    fileplace = project_dict.get('fileplace')
    fdr = project_dict.get('fdr')
    confidenceI = [0.99, 0.95, 0.90][project_dict.get('confidenceI')]
    resfilename = fileplace + '/PM_scores.csv'
...
def plot_scatter_and_tsquared_exe(project_dict, taxalist_plot, networktype, filter_ss):
    fileplace = project_dict.get('fileplace')
    fdr = project_dict.get('fdr')
    confidenceI = [0.99, 0.95, 0.90][project_dict.get('confidenceI')]
    resfilename = fileplace + '/PM_scores.csv'
