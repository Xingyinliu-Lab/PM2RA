...
def plot_scatter_exe(project_dict,filter_ss,topnum,scattertype):
    scatter = scattertype
    ...
    taxnum=np.min([len(pm_res_1D), topnum])
    pm_res_1D=pm_res_1D[0:taxnum]
    ...
    image_pdf = fileplace + '/taxa_difference_between_groups.pdf'
...
def plot_t_squared_exe(project_dict,filter_ss,topnum,scattertype):
    scatter = scattertype
    fileplace = project_dict.get('fileplace')
    fdr = project_dict.get('fdr')
    confidenceI = [0.99, 0.95, 0.90][project_dict.get('confidenceI')]
...
def plot_scatter_and_tsquared_exe(project_dict,filter_ss,topnum,scattertype):
    scatter = scattertype
    fileplace = project_dict.get('fileplace')
    fdr = project_dict.get('fdr')
    confidenceI = [0.99, 0.95, 0.90][project_dict.get('confidenceI')]
